...
find_pkgs() {
  (
  set -e
  cmd < "$(cd "$basedir/$reponame" && find -- * -prune -type d | tr '\n' ' ')"
  )
}
...
find_pkgnames() {
  (
  set -e
  cmd < "$(cd "$basedir/$reponame/$1" && makepkg --printsrcinfo | sed -nr 's/^\W*pkgname = ([-a-zA-Z0-9]+).*$/\1/p' | tr '\n' ' ')"
  )
}
